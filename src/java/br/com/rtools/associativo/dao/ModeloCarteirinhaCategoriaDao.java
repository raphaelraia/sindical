...
public class ModeloCarteirinhaCategoriaDao extends DB {
    ...
    public List findBy(Integer modelo_carteirinha_id, Integer rotina_id) {
        try {
            if(modelo_carteirinha_id == null) {
                Query query = getEntityManager().createQuery("SELECT MCC FROM ModeloCarteirinhaCategoria AS MCC WHERE MCC.rotina.id = :rotina_id ORDER BY MCC.id ASC");
                query.setParameter("rotina_id", rotina_id);
                return query.getResultList();           
            } else {
                Query query = getEntityManager().createQuery("SELECT MCC FROM ModeloCarteirinhaCategoria AS MCC WHERE MCC.modeloCarteirinha.id = :modelo_carteirinha_id AND MCC.rotina.id = :rotina_id ORDER BY MCC.id ASC");
                query.setParameter("modelo_carteirinha_id", modelo_carteirinha_id);
                query.setParameter("rotina_id", rotina_id);
                return query.getResultList();                
            }
     ...
     * @return
     */
    public List findNotInCategoriaByMCC(Integer rotina_id) {
        try {
            String queryString = " "
                    + "     SELECT C.*                                          \n"
