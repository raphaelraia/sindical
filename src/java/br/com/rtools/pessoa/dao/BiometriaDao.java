...
public class BiometriaDao extends DB {
    ...
    public List reloadListBiometria(Integer deviceNumber) {
        try {
            getEntityManager().clear();
            Query query = getEntityManager().createNativeQuery("SELECT b.* FROM pes_biometria AS b WHERE b.is_ativo = true AND (b.ds_biometria IS NOT NULL AND B.ds_biometria <> '') AND B.dt_atualiza_aparelho" + deviceNumber + " IS NOT NULL ORDER BY B.id", Biometria.class);
            List list = query.getResultList();
            if (!list.isEmpty()) {
                return list;
            }
